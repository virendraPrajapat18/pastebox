...
const username = `${cleanedFullname.substring(0, 4).toLowerCase()}${generateUniqueId().substring(0, 5)}`;
    ...
    const avatarLength = nameParts.length > 1 ? 2 : 1;
    const profilePic = `https://ui-avatars.com/api/?name=${encodeURIComponent(fullname)}&background=6366f1&color=fff&bold=true&size=128&length=${avatarLength}`;
...
const getUserById = async (req, res) => {
  ...
  try {
    ...
    // Recalculate stats dynamically from actual files in database
