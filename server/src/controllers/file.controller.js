...
const deleteFile = async (req, res) => {
     ...
     try {
        ...
        if (file.createdBy) {
          const remainingFiles = await File.find({ createdBy: file.createdBy });
          let totalUploads = remainingFiles.length;
          let imageCount = 0;
          let videoCount = 0;
          let documentCount = 0;
...
const incrementDownloadCount = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, downloadCount: file.downloadedContent });
  } catch (error) {
