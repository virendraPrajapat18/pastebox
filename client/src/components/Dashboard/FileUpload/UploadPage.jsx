  ...
  const handleFiles = (fileList) => {
    ...
    Array.from(fileList).forEach((file) => {
      if (file.size > MAX_SIZE) {
        toast.error(`"${file.name}" is too large! Max size is 2GB.`);
      } else {
    ...
    <div className="container bg-[var(--bg-color)] text-[var(--text-color)] p-6 rounded-lg shadow-md">
      ...
      >
        ...
        <div className="dropbox-subtext">
          Supported formats: JPG, PNG, PDF, MP4, MOV, AVI, MKV, ZIP, RAR, ISO, EXE, TAR, GZ (Max 2GB)
        </div>
        <button
        ...
        <input
          ...
          ref={fileInputRef}
          multiple
          accept=".jpg,.jpeg,.webp,.png,.mp4,.avi,.mov,.mkv,.mk3d,.mks,.mka,.pdf,.zip,.rar,.exe,.iso,.tar,.gz"
        ...
        <div className="upload-stats">
          ...
          <div className="progress-bar" style={{ marginTop: "15px" }}>
            <div
              ...
              style={{
                width: `${Math.min(
                  (totalSize / (2 * 1024 * 1024 * 1024)) * 100,
                  100
                )}%`,
              }}
            />
