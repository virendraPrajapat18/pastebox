...
import { API_SERVER_URL } from "../../config/axiosInstance";
...
const GuestFilePreview = ({ guestFiles }) => {
  ...
  const deleteFile = async (fileId) => {
    ...
    try {
      const res = await fetch(`${API_SERVER_URL}/api/files/delete/${fileId}`, {
        method: "DELETE"
      });
      if (res.ok) {
        const updatedFiles = files.filter((file) => file.id !== fileId && file._id !== fileId);
  ...
  const filteredFiles = files?.filter((file) => {
    ...
    const statusMatch = filterStatus
      ? filterStatus === "expired"
        ? file.hasExpiry && file.expiresAt && new Date(file.expiresAt) < new Date()
        : !file.hasExpiry || !file.expiresAt || new Date(file.expiresAt) >= new Date()
      : true;
              ...
              <table className="min-w-full divide-y divide-[var(--border-color)] text-[var(--text-color)]">
                ...
                <tbody className="bg-[var(--bg-color)] divide-y divide-[var(--border-color)]">
                  {paginatedFiles?.map((file) => {
                    ...
                    const isExpired = file.hasExpiry && file.expiresAt ? new Date(file.expiresAt) < new Date() : false;
                        ...
                        >
                          ...
                          <td className="px-6 py-4 flex gap-2 mt-2 text-sm space-x-3">
                            <button
                              onClick={async () => {
                                try {
                                  const shortCode = file.shortUrl.split('/').pop();
                                  const res = await fetch(`${API_SERVER_URL}/api/files/g/${shortCode}`);
                                  if (res.ok) {
                          ...
                          <td className="px-6 py-4 text-sm text-red-500">
                            {file.hasExpiry && file.expiresAt ? (
                              isExpired
                                ? "Expired"
                                : `Expires in ${formatDistanceToNowStrict(new Date(file.expiresAt))}`
                            ) : (
